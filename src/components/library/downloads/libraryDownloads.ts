...
export class LibraryDownloads implements ILibraryDownloads {
    ...
    static from(_downloads: Partial<ILibraryDownloads>, _name: string, _natives: LibraryNatives, _repoURL: string = urls.DEFAULT_LIBS_REPO) {
        ...
        Object.entries(_natives)
            .map(([
                ...
                return {
                    classifier,
                    // os,
                    include: _classifiers.hasOwnProperty(classifier)
                }
            })
    ...
    set artifact(_artifact) { this._artifact = _artifact }
